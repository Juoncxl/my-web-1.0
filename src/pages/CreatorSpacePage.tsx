...
import { countActiveAssetsInFolder } from '../lib/assetSelectors';
...
import type { CreatorWidgetConfig } from '../components/creator/CreatorWidgetEditor';
...
const ProfileEditModal = React.lazy(() => import('../components/ProfileEditModal').then(module => ({ default: module.ProfileEditModal })));
const CreatorWidgetEditor = React.lazy(() => import('../components/creator/CreatorWidgetEditor').then(module => ({ default: module.CreatorWidgetEditor })));
const CreatorNoteEditorModal = React.lazy(() => import('../components/creator/CreatorNoteEditorModal').then(module => ({ default: module.CreatorNoteEditorModal })));
const CreatorGoalEditorModal = React.lazy(() => import('../components/creator/CreatorGoalEditorModal').then(module => ({ default: module.CreatorGoalEditorModal })));
const CreatorMusicEditorModal = React.lazy(() => import('../components/creator/CreatorMusicEditorModal').then(module => ({ default: module.CreatorMusicEditorModal })));
const CreatorGalleryEditorModal = React.lazy(() => import('../components/creator/CreatorGalleryEditorModal').then(module => ({ default: module.CreatorGalleryEditorModal })));
const CreatorDecorationEditorModal = React.lazy(() => import('../components/creator/CreatorDecorationEditorModal').then(module => ({ default: module.CreatorDecorationEditorModal })));
const CreatorClockEditorModal = React.lazy(() => import('../components/creator/CreatorClockEditorModal').then(module => ({ default: module.CreatorClockEditorModal })));
const CreatorWeatherEditorModal = React.lazy(() => import('../components/creator/CreatorWeatherEditorModal').then(module => ({ default: module.CreatorWeatherEditorModal })));
const CreatorCalendarEditorModal = React.lazy(() => import('../components/creator/CreatorCalendarEditorModal').then(module => ({ default: module.CreatorCalendarEditorModal })));
const CreatorTodoEditorModal = React.lazy(() => import('../components/creator/CreatorTodoEditorModal').then(module => ({ default: module.CreatorTodoEditorModal })));
const CreatorFolderEditorModal = React.lazy(() => import('../components/creator/CreatorFolderEditorModal').then(module => ({ default: module.CreatorFolderEditorModal })));
...
function folderAssetCount(folder: Folder, assets: Asset[]) { return countActiveAssetsInFolder(assets, folder.id); }
...
export const CreatorSpacePage: React.FC<CreatorSpacePageProps> = ({ slug, onCreateAsset, onEditAsset, onOpenAuth, onOpenFolderManager, onOpenMoveToFolder, onMoveAssetToFolder, onOpenSettingsModal, allKnownAssets = [], knownFolders = [], isLoadingAssets = false, isLoadingFolders = false, bookmarkedAssetIds = [], likedAssetIds = [], recentlyViewedIds = [], onBookmark, onLike, onDeleteAsset, onRestoreAsset, onPermanentDeleteAsset }) => {
  ...
  const [worksSortMode, setWorksSortMode] = useState<'latest' | 'title'>('latest');
  ...
  const portfolioShowcaseAssets = useMemo(() => {
    if (portfolioDisplayLimit === 'all') return visibleAssets;
    return getPortfolioShowcaseItems(visibleAssets, portfolioDisplayLimit);
  }, [portfolioDisplayLimit, visibleAssets]);
  ...
  const managementAssets = useMemo(() => {
    if (activeTab === 'works') {
      const works = filteredAssets.filter(asset => !asset.deletedAt);
      return worksSortMode === 'title' ? [...works].sort((a, b) => a.title.localeCompare(b.title, 'th')) : works;
    }
    ...
    if (activeTab === 'saved') return selectCreatorSavedAssets(allKnownAssets, bookmarkedAssetIds, currentUser?.id);
    ...
    return [];
  }, [activeTab, allKnownAssets, assets, bookmarkedAssetIds, currentUser?.id, filteredAssets, recentlyViewedIds, visibleAssets, worksSortMode]);
  ...
  const visibleFreePlacements = useMemo(() => {
    ...
    // Public presentation is not the editor canvas: keep the Portfolio
    // showcase as a full-width section so side widgets cannot squeeze cards
    // into an unreadable column. Saved editor coordinates remain untouched.
  ...
  const renderPortfolio = (free = false, placement?: FreeLayoutPlacement) => {
    const hasHiddenShowcaseAssets = portfolioShowcaseAssets.length < visibleAssets.length;
    ...
    return <section key={placement?.id} className={free ? 'csp-portfolio csp-layout-block' : 'csp-portfolio'} data-height-mode={free ? 'auto' : undefined} style={portfolioStyle} {...(placement && canManageFreeLayout ? placementHandlers(placement) : {})}>
      <div ref={free ? portfolioContentRef : undefined} className={free ? 'csp-portfolio-content' : undefined}>
        {isCustomizeOpen && <div className="csp-portfolio-edit-bar"><span>▦ ผลงานล่าสุด · Showcase auto-height</span><div>{layout === 'free' && <select value={placement?.w || spans.portfolio} onChange={event => handleSpanChange('portfolio', Number(event.target.value))} aria-label="ความกว้างผลงานล่าสุด">{getFreePlacementWidthOptions('portfolio').map(value => <option value={value} key={value}>{value} / 12 คอลัมน์</option>)}</select>}<label className="csp-portfolio-limit-label">แสดง<select value={portfolioDisplayLimit} onChange={event => setPortfolioDisplayLimit(event.target.value === 'all' ? 'all' : Number(event.target.value) as PortfolioDisplayLimit)} aria-label="จำนวนผลงานล่าสุด">{[3, 6, 9, 12].map(value => <option value={value} key={value}>{value} ผลงาน</option>)}<option value="all">ทั้งหมด</option></select></label>{layout === 'free' && <><button type="button" onClick={() => moveFreeBlock(getFreePlacementId('portfolio', 'portfolio'), -1)} aria-label="เลื่อนผลงานล่าสุดขึ้น">↑</button><button type="button" onClick={() => moveFreeBlock(getFreePlacementId('portfolio', 'portfolio'), 1)} aria-label="เลื่อนผลงานล่าสุดลง">↓</button>{placement && <button type="button" className="is-danger csp-remove-from-profile" onClick={() => removeCompositionPlacement(placement.id)} title="ซ่อนผลงานล่าสุดจากหน้าโปรไฟล์โดยไม่ลบผลงาน">นำออก</button>}</>}</div></div>}
         <div className="csp-section-heading"><div><p className="csp-eyebrow">SHOWCASE</p><h2>ผลงานล่าสุด</h2><p>ผลงานที่เลือกแสดงบนหน้าโปรไฟล์ของ {displayName}</p></div><span>{portfolioShowcaseAssets.length}{hasHiddenShowcaseAssets ? ` / ${visibleAssets.length}` : ''} รายการ</span></div>
         {isProfileAssetsLoading ? <div className="csp-empty" aria-busy="true"><h3>กำลังโหลดผลงาน...</h3></div> : portfolioShowcaseAssets.length > 0 ? <><div className="csp-asset-grid csp-portfolio-showcase-grid">{portfolioShowcaseAssets.map(asset => <AssetCard key={asset.id} asset={asset} allAssets={allKnownAssets} viewerMode={isEditing ? 'owner' : 'public'} onClick={asset => { void openAssetDetail(asset); }} onLike={!isEditing ? onLike : undefined} isLiked={likedAssetIds.includes(asset.id)} onBookmark={onBookmark} isBookmarked={bookmarkedAssetIds.includes(asset.id)} onDelete={isEditing ? onDeleteAsset : undefined} onEdit={isEditing ? onEditAsset : undefined} isOwner={isEditing} creatorProfile={presentationProfile} presentationMode="profile-compact" />)}</div>{hasHiddenShowcaseAssets && <div className="csp-portfolio-all-action"><button type="button" className="csp-secondary-button" onClick={openAllWorks}>ดูผลงานทั้งหมด</button></div>}</> : <div className="csp-empty"><div className="csp-empty-icon"><Edit3 className="h-5 w-5" /></div><h3>{isEditing ? 'ยังไม่มีผลงานในพื้นที่ของคุณ' : 'Creator คนนี้ยังไม่มีผลงานสาธารณะ'}</h3><p>{isEditing ? 'เริ่มจากผลงานที่มีอยู่ในคลัง หรือเปิด workspace เพื่อเตรียมผลงานใหม่' : 'ผลงานสาธารณะที่ Creator เลือกแสดงจะปรากฏที่นี่'}</p>{isEditing && <button type="button" onClick={handleCreateAsset} className="csp-primary-button"><Plus className="h-4 w-4" />สร้างผลงาน</button>}</div>}
      </div>
    </section>;
  };
