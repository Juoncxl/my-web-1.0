...
export const CreatorSpacePage: React.FC<CreatorSpacePageProps> = ({ slug, onCreateAsset, onOpenAuth, onOpenFolderManager }) => {
  ...
  return <div className="csp-route min-h-screen">
    <Header searchQuery={searchQuery} onSearchChange={setSearchQuery} activeView="feed" onViewChange={() => window.location.assign('/')} onOpenCreateModal={handleCreateAsset} onOpenAuthModal={onOpenAuth} onOpenSignUpModal={() => openAuthModal('signup')} onOpenProfileModal={() => isEditing ? setIsProfileOpen(true) : onOpenAuth()} onOpenSettingsModal={() => isEditing ? setIsProfileOpen(true) : onOpenAuth()} creatorMode />
    <main className="csp-main mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      <div className="csp-breadcrumb"><a href="/">CXL Studio</a><span aria-hidden="true">/</span><span>Creator Space</span></div>
      {isLoading && <section className="csp-loading" aria-label="กำลังโหลด Creator Space"><div /><div className="csp-loading-body"><span /><span /><span /></div></section>}
      {!isLoading && !profile && <section className="csp-empty csp-route-state" role="alert"><div className="csp-empty-icon"><UserRound className="h-6 w-6" /></div><h1>ไม่พบ Creator Space</h1><p>{error || 'โปรไฟล์นี้อาจยังไม่มีอยู่ หรือ URL ไม่ถูกต้อง'}</p><div className="csp-state-actions"><button type="button" onClick={() => void refresh()} className="csp-secondary-button"><RefreshCw className="h-3.5 w-3.5" />ลองใหม่</button><a href="/" className="csp-primary-button">กลับหน้าแรก</a></div></section>}
      {!isLoading && profile && <>
