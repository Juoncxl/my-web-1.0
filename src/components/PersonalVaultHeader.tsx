...
import { User, Edit3, Lock, Globe, Sparkles, Shield, Plus, Calendar, Folder as FolderIcon, FolderPlus, Settings2 } from 'lucide-react';
...
export const PersonalVaultHeader: React.FC<PersonalVaultHeaderProps> = ({
  ...
  return (
    <div className="space-y-4 mb-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="relative shrink-0">
            <img
              ...
              alt="Avatar"
              className="w-24 h-24 sm:w-28 sm:h-28 rounded-full object-cover ring-4 ring-white dark:ring-slate-800 shadow-md"
            />
          ...
          <div className="flex-1 text-center md:text-left space-y-3">
            ...
            <div className="flex items-center justify-center md:justify-start gap-4 pt-1">
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xs px-4 py-2 rounded-2xl border border-purple-100/90 dark:border-purple-900/60 text-center min-w-[90px]">
                <div className="text-lg font-extrabold text-slate-800 dark:text-white">{totalAssetsCount}</div>
                <div className="text-[10.5px] font-semibold text-slate-500 dark:text-slate-400">ผลงานทั้งหมด</div>
              </div>

              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xs px-4 py-2 rounded-2xl border border-purple-100/90 dark:border-purple-900/60 text-center min-w-[90px]">
                <div className="text-lg font-extrabold text-indigo-600 dark:text-indigo-400 flex items-center justify-center gap-1">
                  <Globe className="w-3.5 h-3.5" />
                  <span>{publicCount}</span>
                </div>
                <div className="text-[10.5px] font-semibold text-slate-500 dark:text-slate-400">สาธารณะ</div>
              </div>
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xs px-4 py-2 rounded-2xl border border-purple-100/90 dark:border-purple-900/60 text-center min-w-[90px]">
                ...
                <div className="text-[10.5px] font-semibold text-slate-500 dark:text-slate-400">ส่วนตัว (Locked)</div>
              </div>
      ...
      <div className="p-3.5 bg-white dark:bg-slate-800/90 rounded-2xl border border-purple-100 dark:border-purple-900/60 shadow-xs">
        <div className="flex items-center justify-between gap-2 mb-2 px-1">
          <div className="flex items-center gap-2">
            <FolderIcon className="w-4 h-4 text-purple-600 dark:text-purple-400" />
            <span className="text-xs font-bold text-slate-800 dark:text-slate-100">
              โฟลเดอร์ของฉัน (Folders):
            </span>
          </div>
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all flex items-center gap-1.5 ${
              selectedFolderId === 'all'
                ...
                : 'bg-slate-50 dark:bg-slate-700/60 text-slate-700 dark:text-slate-300 hover:bg-purple-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'
            }`}
          >
            <span>📁 ผลงานทั้งหมด</span>
            <span className={`px-1.5 py-0.2 rounded-full text-[10px] ${selectedFolderId === 'all' ? 'bg-purple-700 text-white' : 'bg-slate-200 dark:bg-slate-600'}`}>
              {totalAssetsCount}
            </span>
          </button>
            ...
            return (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all flex items-center gap-1.5 border ${
                  isSelected
                    ? 'bg-purple-600 dark:bg-purple-600 text-white border-purple-600 font-bold shadow-xs'
