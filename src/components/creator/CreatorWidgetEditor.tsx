...
import type { CreatorWidgetConfig } from './creatorWidgetModel';

export type { CreatorWidgetConfig } from './creatorWidgetModel';
...
export const CreatorWidgetEditor: React.FC<CreatorWidgetEditorProps> = ({ type, config, displayName, contextual = false, instanceId, onChange, onDisplayNameChange, onClose }) => {
  ...
  return <section className={`csp-widget-editor ${contextual ? 'is-contextual' : ''}`} data-widget-editor-instance-id={instanceId} aria-label={`แก้ไข ${CREATOR_WIDGET_LABELS[type]}`}>
    <div className="csp-widget-editor-heading"><strong>{CREATOR_WIDGET_ICONS[type]} แก้ไข {CREATOR_WIDGET_LABELS[type]}</strong><span>การตั้งค่านี้อยู่ใน session เท่านั้น · ยังไม่เขียน database</span></div>
    <div className="csp-widget-editor-fields">
      <label className="csp-field">ชื่อแสดง Widget<input value={displayName ?? ''} placeholder={CREATOR_WIDGET_LABELS[type]} onChange={event => onDisplayNameChange?.(event.target.value)} /></label>
