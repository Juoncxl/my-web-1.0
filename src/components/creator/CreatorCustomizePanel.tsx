...
export type CreatorWidgetType = 'folder' | 'playlist' | 'todo' | 'status' | 'note' | 'links' | 'goal' | 'gallery' | 'clock' | 'calendar' | 'single_image' | 'decoration' | 'featured_work';
...
const WIDGET_LIBRARY: Array<{ group: string; items: Array<{ type: CreatorWidgetType; icon: string; label: string }> }> = [
  ...
  { group: 'สื่อและ Utility', items: [{ type: 'playlist', icon: '🎵', label: 'Playlist' }, { type: 'single_image', icon: '🌄', label: 'รูปภาพเดี่ยว' }, { type: 'gallery', icon: '🖼️', label: 'แกลเลอรี' }, { type: 'featured_work', icon: '⭐', label: 'ผลงานเด่น' }, { type: 'decoration', icon: '✦', label: 'Decoration' }, { type: 'clock', icon: '🕒', label: 'นาฬิกา' }] }
];

export const CREATOR_WIDGET_LABELS: Record<CreatorWidgetType, string> = {
  folder: 'โฟลเดอร์', playlist: 'Playlist', todo: 'สิ่งที่ต้องทำ', status: 'สถานะ', note: 'โน้ต', links: 'ลิงก์ของฉัน', goal: 'เป้าหมาย / ความคืบหน้า', gallery: 'แกลเลอรี', clock: 'นาฬิก', calendar: 'ปฏิทิน', single_image: 'รูปภาพเดี่ยว', decoration: 'Decoration', featured_work: 'ผลงานเด่น'
};

export const CREATOR_WIDGET_ICONS: Record<CreatorWidgetType, string> = {
  folder: '📁', playlist: '🎵', todo: '✅', status: '💭', note: '📝', links: '🔗', goal: '🎯', gallery: '🖼️', clock: '🕒', calendar: '📅', single_image: '🌄', decoration: '✦', featured_work: '⭐'
};
...
export const CreatorCustomizePanel: React.FC<CreatorCustomizePanelProps> = ({
  ...
  return (
    <>
      <section className="csp-customize-toolbar" aria-label="เครื่องมือตกแต่ง Creator Space">
        <div><p className="csp-eyebrow">CUSTOMIZE MODE</p><strong><Sparkles className="inline h-4 w-4" /> กำลังตกแต่ง Creator Space</strong><span>การจัดวางนี้เก็บใน session เท่านั้น · ยังไม่เขียน database</span></div>
        <div className="csp-customize-actions"><button type="button" className="csp-secondary-button" onClick={() => setLibraryOpen(true)}><Plus className="h-3.5 w-3.5" />เพิ่มบล็อก</button><button type="button" className="csp-secondary-button" onClick={() => setLayoutOpen(value => !value)}><Grid2X2 className="h-3.5 w-3.5" />{layout === 'free' ? 'Free · 12-column' : `Locked ${lockedPreset === 'left' ? 'A' : lockedPreset === 'right' ? 'B' : 'C'}`}</button><button type="button" className="csp-primary-button" onClick={onClose}>เสร็จสิ้น</button></div>
      </section>
      ...
      <div className="csp-customize-help"><Settings2 className="h-4 w-4" /><span>{layout === 'free' ? 'Free: ลาก/เรียงใน canvas และปรับ span ได้ที่แถบของแต่ละ block' : 'Locked: โครงหลักคงที่ · ใช้ ↑ ↓ เรียง widget และปุ่ม rail ใน Locked C'}</span></div>
...
export const CreatorWidgetControls: React.FC<CreatorWidgetControlsProps> = ({ type, layout, lockedPreset, span, rail, onMove, onEdit, onRemove, onRail, onSpan }) => (
  <div className="csp-widget-edit-bar"><span className="csp-drag-handle" aria-hidden="true">⋮⋮</span><strong>{CREATOR_WIDGET_ICONS[type]} {CREATOR_WIDGET_LABELS[type]}</strong>{layout === 'free' && <select value={span} onChange={event => onSpan(Number(event.target.value))} aria-label={`ความกว้าง ${CREATOR_WIDGET_LABELS[type]}`}>{SPAN_OPTIONS.map(value => <option value={value} key={value}>{spanLabel(value)}</option>)}</select>}<button type="button" onClick={() => onMove(-1)} aria-label={`เลื่อน ${CREATOR_WIDGET_LABELS[type]} ขึ้น`}><ChevronUp className="h-3.5 w-3.5" /></button><button type="button" onClick={() => onMove(1)} aria-label={`เลื่อน ${CREATOR_WIDGET_LABELS[type]} ลง`}><ChevronDown className="h-3.5 w-3.5" /></button>{layout === 'locked' && lockedPreset === 'split' && <><button type="button" className={rail === 'left' ? 'is-selected' : ''} onClick={() => onRail('left')} aria-label="ย้ายไป rail ซ้าย">←</button><button type="button" className={rail === 'right' ? 'is-selected' : ''} onClick={() => onRail('right')} aria-label="ย้ายไป rail ขวา">→</button></>}<button type="button" onClick={onEdit} aria-label={`แก้ไข ${CREATOR_WIDGET_LABELS[type]}`}><Settings2 className="h-3.5 w-3.5" /></button><button type="button" className="is-danger" onClick={onRemove} aria-label={`ลบ ${CREATOR_WIDGET_LABELS[type]} ออกจาก layout`}>×</button></div>
);
