  ...
  return (
    <>
      <section className="csp-customize-toolbar" aria-label="เครื่องมือตกแต่งโปรไฟล์">
        ...
        <div className="csp-customize-actions"><button type="button" className="csp-secondary-button" onClick={() => setLibraryOpen(true)}><Plus className="h-3.5 w-3.5" />เพิ่มวิดเจ็ต</button><button type="button" className="csp-secondary-button" onClick={() => setLayoutOpen(value => !value)}><Grid2X2 className="h-3.5 w-3.5" />{layout === 'free' ? 'Free · 12-column' : `Locked ${lockedPreset === 'left' ? 'A' : lockedPreset === 'right' ? 'B' : 'C'}`}</button><button type="button" className="csp-primary-button" onClick={onClose}>เสร็จสิ้น</button></div>
      </section>
