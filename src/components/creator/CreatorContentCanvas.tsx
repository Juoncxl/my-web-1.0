...
const ContentSection: React.FC<ContentSectionProps> = ({
  ...
  if (type === 'image_prompt') {
    return <article className="csp-content-canvas-section" data-content-section={type}>
      ...
      <div className="csp-content-example-images"><div className="csp-content-section-heading"><div><h4>รูปตัวอย่าง</h4><p>เพิ่มภาพอ้างอิงหรือผลลัพธ์ตัวอย่างไว้กับคำสั่งเจนรูปนี้</p></div><span>{draft.imagePrompt.exampleImages.length}/6 รูป</span></div><label className="csp-secondary-button csp-file-button"><ImagePlus className="h-4 w-4" />เลือกภาพตัวอย่าง<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={event => { const file = event.target.files?.[0]; if (!file || !isSupportedCanvasImage(file)) return; onExampleImagesChange([...draft.imagePrompt.exampleImages, URL.createObjectURL(file)]); event.target.value = ''; }} /></label>{draft.imagePrompt.exampleImages.length > 0 && <div className="csp-content-example-strip">{draft.imagePrompt.exampleImages.map((image, index) => <button type="button" key={`${image.slice(0, 18)}-${index}`} onClick={() => onExampleImagesChange(draft.imagePrompt.exampleImages.filter((_, itemIndex) => itemIndex !== index))} aria-label={`ลบภาพตัวอย่างที่ ${index + 1}`}><img src={image} alt="" /></button>)}</div>}</div>
    </article>;
  }
