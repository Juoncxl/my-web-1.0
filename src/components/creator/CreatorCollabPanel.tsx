...
type SharedInformationDropTarget = { id: string; position: 'before' | 'after' };
...
const SharedInformationItem: React.FC<SharedInformationItemProps> = ({ item, expanded, index, total, dragging, dropPosition, counterMode, platformOptions, onToggle, onChange, onRemove, onExpand, onMove, onDragStart, onDragMove, onDragEnd }) => {
  ...
  const title = item.title.trim() || 'หัวข้อที่ยังไม่ได้ตั้งชื่อ';
  return <article className={`csp-collab-shared-item ${expanded ? 'is-expanded' : ''} ${dragging ? 'is-dragging' : ''} ${dropPosition ? `is-drop-${dropPosition}` : ''}`} data-shared-information-id={item.id}>
    <div className="csp-collab-shared-summary">
      <div className="csp-collab-shared-reorder-controls">
        <button type="button" className="csp-collab-shared-drag-handle" aria-label={`ลากเพื่อจัดตำแหน่ง ${title}`} title="ลากเพื่อจัดลำดับ" onPointerDown={onDragStart} onPointerMove={onDragMove} onPointerUp={event => onDragEnd(event, false)} onPointerCancel={event => onDragEnd(event, true)}><GripVertical aria-hidden="true" /></button>
        <span className="csp-collab-shared-move-buttons">
          <button type="button" onClick={() => onMove(-1)} disabled={index === 0} aria-label={`เลื่อน ${title} ขึ้น`} title="เลื่อนขึ้น"><ChevronUp aria-hidden="true" /></button>
          <button type="button" onClick={() => onMove(1)} disabled={index === total - 1} aria-label={`เลื่อน ${title} ลง`} title="เลื่อนลง"><ChevronDown aria-hidden="true" /></button>
        </span>
      </div>
      <button type="button" className="csp-collab-shared-toggle" onClick={onToggle} aria-expanded={expanded} aria-controls={`csp-collab-shared-detail-${item.id}`}><span><strong>{item.title.trim() || 'ยังไม่ได้ตั้งชื่อหัวข้อ'}</strong><small>{item.type === 'code' ? 'โค้ด' : 'ข้อความ'} · {scopeLabel}</small></span><ChevronDown aria-hidden="true" /></button>
      <button type="button" className="csp-collab-detail-button" onClick={onToggle}>{expanded ? 'ปิด' : 'ดูรายละเอียด'}</button>
    </div>
...
export const CreatorCollabPanel: React.FC<CreatorCollabPanelProps> = ({ draft, visibility, onVisibilityChange, platformOptions, counterMode, onCounterModeChange, creatorProfile = null, ownedWorks = [], currentWorkId, onChange }) => {
  ...
  const [draggedSharedId, setDraggedSharedId] = useState<string | null>(null);
  const [sharedDropTarget, setSharedDropTarget] = useState<SharedInformationDropTarget | null>(null);
  const [sharedReorderStatus, setSharedReorderStatus] = useState('');
  ...
  const setSharedDrop = (target: SharedInformationDropTarget | null) => {
    const current = sharedDropTargetRef.current;
    if (current?.id === target?.id && current?.position === target?.position) return;
    sharedDropTargetRef.current = target;
    setSharedDropTarget(target);
  };
  const startSharedInformationDrag = (itemId: string, event: React.PointerEvent<HTMLButtonElement>) => {
    if (event.button !== 0) return;
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    draggedSharedIdRef.current = itemId;
    setDraggedSharedId(itemId);
    setSharedDrop(null);
    setSharedReorderStatus('');
  };
  const moveSharedInformationDrag = (event: React.PointerEvent<HTMLButtonElement>) => {
    const itemId = draggedSharedIdRef.current;
    if (!itemId) return;
    const targetElement = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>('[data-shared-information-id]');
    const targetId = targetElement?.dataset.sharedInformationId;
    if (!targetElement || !targetId || targetId === itemId) {
      setSharedDrop(null);
      return;
    }
    const rect = targetElement.getBoundingClientRect();
    setSharedDrop({ id: targetId, position: event.clientY < rect.top + rect.height / 2 ? 'before' : 'after' });
  };
  const finishSharedInformationDrag = (event: React.PointerEvent<HTMLButtonElement>, cancelled: boolean) => {
    const itemId = draggedSharedIdRef.current;
    const target = sharedDropTargetRef.current;
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
    if (!cancelled && itemId && target) {
      const next = reorderCollabSharedInformation(draft, itemId, target.id, target.position);
      onChange(next);
      const position = next.sharedInformation.findIndex(item => item.id === itemId) + 1;
      const title = next.sharedInformation.find(item => item.id === itemId)?.title.trim() || 'หัวข้อที่ยังไม่ได้ตั้งชื่อ';
      setSharedReorderStatus(`ย้าย ${title} ไปตำแหน่งที่ ${position} แล้ว`);
    } else if (cancelled && itemId) {
      setSharedReorderStatus('ยกเลิกการย้ายแล้ว ลำดับเดิมไม่เปลี่ยน');
    }
    draggedSharedIdRef.current = null;
    setDraggedSharedId(null);
    setSharedDrop(null);
  };
  const moveSharedInformationBy = (itemId: string, offset: -1 | 1) => {
    const itemIndex = draft.sharedInformation.findIndex(item => item.id === itemId);
    const target = draft.sharedInformation[itemIndex + offset];
    if (itemIndex < 0 || !target) return;
    const next = reorderCollabSharedInformation(draft, itemId, target.id, offset < 0 ? 'before' : 'after');
    onChange(next);
    const title = next.sharedInformation.find(item => item.id === itemId)?.title.trim() || 'หัวข้อที่ยังไม่ได้ตั้งชื่อ';
    setSharedReorderStatus(`ย้าย ${title} ไปตำแหน่งที่ ${itemIndex + offset + 1} แล้ว`);
  };
